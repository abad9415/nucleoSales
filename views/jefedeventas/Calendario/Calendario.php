...
require_once('../../../views/Calendario/bdd.php');
  ...
  $consultarProspectos = $prospectos->consultarProspectos(); 
	...
	$ConsultasCalendario = new EventosC($datosConexionBD);
 	$llenarCalendarioV = $ConsultasCalendario->llenarCalendarioV();  
...
?>
	...
	.col-centered{
		...
		margin: 10 auto;
			...
			.modal-dialog {
          height: 300px; /* New width for default modal */
        }
    ...
    <div>
		...
		<!-- editmodal -->
				...
				<!-- agregar modal -->
			<div class="modal fade" id="ModalAddVendedor" tabindex="-1" role="dialog" data-target=".bd-example-modal-lg" aria-labelledby="myModalLabel">
		  <div class="modal-dialog" role="document">
			<div class="modal-content">
			<form id="GuardarCita" class="form-horizontal">
			  ...
			  <div class="modal-body modal-dialog">
					...
					<label for="title" class="col-sm-2 control-label">Tipo</label>
						<div class=col-sm-10>
						<select name="tipoCita" id="tipoCita"class="form-control" >							
							
							<option value=3>Vendedor-Prospecto</option>			
						</select>
						</div>
					</div>
					...
					<label for="title" class="col-sm-2 control-label">Descripcion:</label>
					<div class=col-sm-10>
					  <input type="text" name="desCita" class="form-control" id="desCita" placeholder="Citas">
						</div>
				  </div>
           ...
           while($row = $consultarProspectos->fetch_assoc()) { ?>
					...
					<div class="contacto col-sm-10">
					  <select name="contacto" class="form-control" id="contacto" disabled="disabled">
						</select>
						</div>
						</div>
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			select: function(start, end) {
				...
				$('#ModalAddVendedor #deFecha').val((moment(start).format('YYYY-MM-DD')));
				$('#ModalAddVendedor #aFecha').val((moment(start).format('YYYY-MM-DD')));
				
				$('#ModalAddVendedor').modal('show');
			...
			eventRender: function(event, element) {
				...
				element.bind('dblclick', function() {
					...
					$.ajax({
			 		url: '../../views/jefedeventas/Calendario/ContCal.php',
			 		type: "POST",
			 		data: "id="+event.id,
			...
			events: [
			<?php 
              while($row = $llenarCalendarioV->fetch_assoc()) { 
                  ?>
				{
					id: '<?php echo $row['id']; ?>',
...
<script src="../../views/jefedeventas/Calendario/js/borrar.js"></script>